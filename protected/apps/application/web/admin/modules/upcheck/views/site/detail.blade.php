...
@section('content')
        ...
        <div class="col-xs-9">
            @foreach($images as $item )
                <div class="col-xs-3">
                    ...
                    @else
                        <a href="{{env('QINIU_DOMAIN')}}/{{$item['image']}}" target="_blank"> <img
                                    src="{{env('QINIU_DOMAIN')}}/{{$item['image']}}" width="200"/> </a>
                    @endif
                </div>
            @endforeach
        </div>
    ...
    <div class="col-xs-12">
        ...
        <div class="col-md-6">
            <input class="form-control " type="text" name="check_doctor" id="check_doctor"
                   value="{{$checkData['check_doctor']}}" placeholder="检测医生">
        </div>
    </div>
    ...
    <div class="col-xs-12">
        ...
        <div class="col-md-6">
            <textarea class="form-control " cols="9" rel="3" id="check_desc">{{$checkData['check_desc']}}</textarea>
        </div>
        <label class="col-md-3 control-label">
            <input type="button" class="input-group-btn btn btn-default btn-sm input-small save" value="保存"
                   style="background: #3fd5c0;color: #ffffff;"> </label>
    </div>
