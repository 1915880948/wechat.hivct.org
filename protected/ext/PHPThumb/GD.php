...
class GD extends PHPThumb
{
     ...
     * @param array $plugins
     */
    public function __construct($fileName, $options = [], array $plugins = [])
    {
    ...
    public function adjustOrientation()
    {
        if(function_exists('exif_read_data')){
            $w0 = $this->currentDimensions['width'];
            $h0 = $this->currentDimensions['height'];
            $exif = @exif_read_data($this->fileName, EXIF);
            if(isset($exif['Orientation'])){
                ...
                if($degree > 0){
                    $this->oldImage = imagerotate($this->oldImage, $degree, 0);
                }
                if($mirror){
                    $nm = $this->oldImage;
                    $this->oldImage = imagecreatetruecolor($w0, $h0);
                    imagecopyresampled($this->oldImage, $nm, 0, 0, $w0 - 1, 0, $w0, $h0, -$w0, $h0);
                    imagedestroy($nm);
                }
            }
            /**
