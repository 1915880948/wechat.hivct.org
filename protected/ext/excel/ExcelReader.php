...
class ExcelReader
{
    ...
    {
        $col = $this->getCol($col);
        if(array_key_exists('cellsInfo', $this->sheets[$sheet]) && array_key_exists($row, $this->sheets[$sheet]['cellsInfo']) &&
           array_key_exists($col, $this->sheets[$sheet]['cellsInfo'][$row]) && array_key_exists($type, $this->sheets[$sheet]['cellsInfo'][$row][$col])
        ){
    ...
    {
        return isset($this->rowInfo[$sheet][$row]['hidden']) ? !!$this->rowInfo[$sheet][$row]['hidden'] : 0;
    }
    ...
    {
        return isset($this->colors[$this->xfProperty($row, $col, $sheet, 'borderLeftColor')])
            ? $this->colors[$this->xfProperty($row, $col, $sheet, 'borderLeftColor')] : "";
    }

    function borderRightColor($row, $col, $sheet = 0)
    {
        return isset($this->colors[$this->xfProperty($row, $col, $sheet, 'borderRightColor')])
            ? $this->colors[$this->xfProperty($row, $col, $sheet, 'borderRightColor')] : "";
    }

    function borderTopColor($row, $col, $sheet = 0)
    {
        return isset($this->colors[$this->xfProperty($row, $col, $sheet, 'borderTopColor')])
            ? $this->colors[$this->xfProperty($row, $col, $sheet, 'borderTopColor')] : "";
    }

    function borderBottomColor($row, $col, $sheet = 0)
    {
        return isset($this->colors[$this->xfProperty($row, $col, $sheet, 'borderBottomColor')])
            ? $this->colors[$this->xfProperty($row, $col, $sheet, 'borderBottomColor')] : "";
    }
     ...
     * @param string $file
     * @param bool $store_extended_info
