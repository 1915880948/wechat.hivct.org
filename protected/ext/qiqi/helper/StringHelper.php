...
class StringHelper extends \yii\helpers\StringHelper
{
     ...
     * @return array
     */
    static function normalize($str, $delimiter = ".")
    {
        if(is_array($str)){
            return $str;
        }

        return array_filter(array_map("trim", explode($delimiter, $str)), 'strlen');
    }

    static public function createGUID($data = null)
    {
        if($data !== null){
        ...
        return vsprintf('%04X%04X-%04X-%04X-%04X-%04X%04X%04X', [
            mt_rand(0, 65535),
            mt_rand(0, 65535),
            mt_rand(0, 65535),
            mt_rand(16384, 20479),
     ...
     * @return string
     */
    public static function analyzeContent($content, $filterEmpty = true)
    {
        $array = [];
        $tmp = str_replace([
            '<br/>',
        ...
        if(!empty($list)){
            foreach($list as $value){
                ...
                $array[] = '' . preg_replace(['/\t/is'], '', str_replace([
                        '&nbsp;',
                        '　',
                    ], '', preg_replace('/\s*/is', '', $value)));
            }
        }
     ...
     * @return mixed
     */
    public static function hideStringInfo($string, $start = 2, $end = -2)
    {
        return substr_replace($string, '****', $start, $end > 0 ? -($end) : $end);
    }
     ...
     * @return string
     */
    public static function hideUserInfo($info, $start = 2, $end = -2)
    {
        return self::truncate($info, $start, "***") . mb_substr($info, ($end > 0 ? -($end) : $end), abs($end), 'UTF-8');
    }

    static function truncate($string, $length, $suffix = '...', $encoding = 'UTF-8', $asHtml = false)
    {
        return parent::truncate($string, $length, $suffix, $encoding, $asHtml);
    }
     ...
     * @return string
     */
    static function substring($string, $length, $havedot = 0)
    {
        //判断长度
     ...
     * @return bool
     */
    public static function endsWith2($haystack, $needles)
    {
     ...
     * @return bool
     */
    public static function startsWith2($haystack, $needles)
    {
        foreach((array) $needles as $needle){
    ...
    static function removeScriptTag($string, $tag = '')
    {
        if(!$tag){
            return $string;
        }
    ...
    /**
     * 将UTF8字符串切割成单字数组，支持中文
     * @param string $string
     * @param        $length
     * @return array
     */
    public static function utf8Split($string, $length = 0)
    {
        ...
        for($i = 0; $i < $strLength; $i++){
            ...
            $result[] = $retstr;
            if($length > 0 && count($result) == $length){
                return $result;
            }
        }
        return $result;
